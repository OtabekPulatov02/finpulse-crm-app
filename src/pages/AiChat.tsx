...
import {
  aiAgent, clearAllAiChats, deleteAiChat, fetchAiChat, fetchAiChats,
...
export default function AiChat() {
  ...
  const clearAll = async () => {
    ...
    if (!window.confirm("Удалить все диалоги без возможности восстановления?")) return;
    await clearAllAiChats().catch(() => {});
    setChats([]);
    newChat();
  };
      ...
      <Card className="flex w-60 shrink-0 flex-col max-md:hidden">
        <div className="flex items-center gap-1.5 p-3">
          <button onClick={newChat}
            className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-brand-600 px-3 py-2 text-[13px] font-medium text-white hover:bg-brand-700">
            <MessageSquarePlus className="size-4" /> Новый диалог
          </button>
          {!!chats.length && (
            <button onClick={clearAll} title="Очистить все диалоги"
              className="flex shrink-0 items-center justify-center rounded-lg border border-slate-200 p-2 text-slate-400 hover:bg-red-50 hover:text-red-600">
