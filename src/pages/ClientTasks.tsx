...
import { Hash, Kanban, LayoutList, Paperclip, Pencil, Plus } from "lucide-react";
import { ClientHeader } from "../components/ClientHeader";
import { Field, Input, Modal, Textarea, toast } from "../components/ui";
import {
  createTaskRequest, fetchBotTasks, fmtTs, updateTaskRequest, type BotTask,
} from "../api";
  ...
  async function submit() {
    ...
    try {
      if (edit && task) {
        ...
        toast("Задача создана — бухгалтер получит её и назначит исполнителя");
        onSaved(r.task);
      }
      onClose();
    } finally {
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-6">
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          ...
          <button onClick={openCreate}
            className="flex items-center gap-2 rounded-lg bg-brand-600 px-3.5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-brand-700">
            <Plus className="size-4" /> Новая задача
          </button>
        </div>

        <div className="mb-4 flex gap-1.5 rounded-lg bg-slate-100 p-1 max-w-max">
        ...
        {tasks !== null && tasks.length > 0 && view === "kanban" && (
          <div className="grid grid-cols-3 gap-3 max-md:grid-cols-1">
            {COLUMNS.map(({ status, dot }) => {
              ...
              return (
                <div key={status} className="rounded-xl bg-slate-100/70 p-2.5">
